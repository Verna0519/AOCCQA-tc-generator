...
description: >
  ...
  預設為可複製的 Markdown 表格,供下游 aoccqa-testcase-reviewer 審查、aoccqa-case-exporter 匯出。
...
**上游**:`aoccqa-fsd-parser`(需求分析報告 / Requirement Matrix)。**下游**:`aoccqa-testcase-reviewer`(審查、刪重、補漏)→ `aoccqa-case-exporter`(輸出 XLSX／HTML)。
